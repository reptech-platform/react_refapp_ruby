    ...
    return (
        <>
            <Box sx={{ float: "left", minWidth: "95%", margin: 2 }}>
                <CustomTable {...props} title={title} mode={"view"} configData={row} rows={dataRows} onTableRowUpdated={onTableRowUpdated} />
            </Box>
        </>
    )
}
    ...
    return (
        <>
            <Box sx={{ float: "left", minWidth: "95%", margin: 2, boxShadow, borderRadius }}>
                <RenderFormContols shadow={true} mode={"view"} location={location} title={title} onInputChange={OnInputChange}
                    controls={row} />
            </Box>
        </>
    )
...
const Component = (props) => {
    ...
    const fetchData = async () => {
        ...
        await Extract(id).then(rslt => {
            const { collections, items, row, options } = rslt;
